...
class Database:
    ...
    def delete_monitor(self, username: str, owner_id: int | None = None) -> None:
        username = username.strip().replace("@", "").lower()
        if self.enabled:
            query = {"username": username}
            if owner_id is not None:
                query["owner_id"] = int(owner_id)
            self.db.monitored_profiles.delete_many(query)
        else:
            keys = []
            for k, v in self.local_monitors.items():
                if v.get("username") == username and (owner_id is None or v.get("owner_id") == int(owner_id)):
                    keys.append(k)
            for k in keys:
