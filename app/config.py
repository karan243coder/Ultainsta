...
class Settings:
    ...
    monitor_interval_seconds: int = _int("MONITOR_INTERVAL_SECONDS", 1800)
    monitor_concurrency: int = _int("MONITOR_CONCURRENCY", 3)
    monitor_max_new_items_per_profile: int = _int("MONITOR_MAX_NEW_ITEMS_PER_PROFILE", 30)
    monitor_keep_seen_limit: int = _int("MONITOR_KEEP_SEEN_LIMIT", 300)
    monitor_initial_delay_seconds: int = _int("MONITOR_INITIAL_DELAY_SECONDS", 20)
