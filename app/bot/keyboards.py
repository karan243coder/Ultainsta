...
def monitor_menu(prefix: str = "user_mon") -> InlineKeyboardMarkup:
    # Admin panel me Back admin main menu par jayega.
    # Normal user /monitor menu me Back/Close message ko close karega.
    back_text = "↩️ Back to Admin" if prefix == "admin_mon" else "❌ Close"
    back_data = "admin_main" if prefix == "admin_mon" else "user_mon_close"
    rows = [
    ...
    if prefix == "admin_mon":
        rows.append([InlineKeyboardButton(text="📊 Monitor Stats", callback_data="admin_mon_stats"), InlineKeyboardButton(text="🔍 Search User ID", callback_data="admin_mon_search")])
    rows.append([InlineKeyboardButton(text="❌ Remove Profile", callback_data=f"{prefix}_del"), InlineKeyboardButton(text=back_text, callback_data=back_data)])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def monitor_delete_menu(monitors: list[dict], prefix: str = "user_mon") -> InlineKeyboardMarkup:
    rows = []
    for m in monitors:
        raw = m.get("username") or m.get("_id", "")
        uname = str(raw).split(":")[-1].replace("@", "").strip().lower()
        if not uname:
            continue
