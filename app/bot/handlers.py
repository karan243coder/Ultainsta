...
def format_monitor_owner(owner_id: int) -> str:
    info = db.get_user_info(owner_id)
    first_name = h(info.get("first_name") or "Unknown")
    tg_username = info.get("username") or "NoUsername"
    username_part = f"@{h(tg_username)}" if tg_username and tg_username != "NoUsername" else "@NoUsername"
    return f"{first_name} ({username_part}) | ID: <code>{owner_id}</code>"


def format_monitor_list(monitors: list[dict], is_admin_panel: bool) -> str:
    txt = "🔄 <b>Monitored Profiles</b>\n━━━━━━━━━━━━━━━━━━━━\n"
    if not monitors:
        return txt + "💡 No profiles are currently monitored."
    for i, m in enumerate(monitors, 1):
        uname = monitor_username(m)
        owner = int(m.get("owner_id") or m.get("admin_id") or 0)
        cached = len(m.get("last_seen_items", []) or [])
        txt += f"{i}. @{h(uname)}\n"
        if is_admin_panel and owner:
            txt += f"   👤 {format_monitor_owner(owner)}\n"
        txt += f"   📌 Cached: <code>{cached}</code>\n"
    return txt.rstrip()
...
async def monitor_callbacks(call: CallbackQuery):
    ...
    if action == "list":
        ...
        txt = format_monitor_list(mons, is_admin_panel)
    ...
    if action == "stats":
        ...
        top_lines = []
        for owner, count in sorted(per_user.items(), key=lambda x: x[1], reverse=True)[:10]:
            top_lines.append(f"• {format_monitor_owner(owner)} → <code>{count}</code>")
        txt = (
            "📊 <b>Auto-Monitor Stats</b>\n━━━━━━━━━━━━━━━━━━━━\n"
            f"🔄 Total Profiles: <code>{stats['total_monitors']}</code>\n"
            f"👥 Unique Users: <code>{stats['unique_users']}</code>\n\n"
            "🏆 <b>Top Monitor Users:</b>\n" + ("\n".join(top_lines) if top_lines else "No data")
        )
        await safe_edit(call.message, txt, reply_markup=monitor_menu(prefix), parse_mode="HTML")
        await safe_answer(call)
        return
    ...
    if action == "search":
        if not is_admin_panel:
            await safe_answer(call, "Admin only", show_alert=True)
            return
        PENDING_ACTIONS[call.from_user.id] = "admin_monitor_search"
        await call.message.answer("🔍 <b>Send Telegram User ID</b> jiska auto-monitor list dekhna hai.", parse_mode="HTML")
        await safe_answer(call, "Send User ID")
        return
    ...
    if action.startswith("remove:"):
        ...
        deleted = db.delete_monitor(uname, None if is_admin_panel else call.from_user.id)
        mons = db.list_monitors(owner_id)
        if deleted:
            txt = f"✅ <b>Stopped monitoring @{h(uname)}</b>\nDeleted: <code>{deleted}</code>\n\n🔄 <b>Remaining Profiles</b>\n━━━━━━━━━━━━━━━━━━━━\n"
            alert = f"Stopped @{uname}"
        else:
            txt = f"⚠️ <b>@{h(uname)} not found or already removed.</b>\n\n🔄 <b>Current Profiles</b>\n━━━━━━━━━━━━━━━━━━━━\n"
            alert = f"Not found @{uname}"
        if mons:
            ...
            txt += "No monitors."
        await safe_edit(call.message, txt, reply_markup=monitor_menu(prefix), parse_mode="HTML")
        await safe_answer(call, alert, show_alert=True)
        return
...
async def catch_all(message: Message, bot: Bot):
    ...
    if pending == "api_add":
        if not is_admin:
            await message.answer("❌ Only admin can add API nodes.")
            return
        clean_url = text.strip().rstrip("/")
        if not ((clean_url.startswith("http://") or clean_url.startswith("https://")) and "." in clean_url) or "instagram.com" in clean_url:
            await message.answer("❌ Invalid API URL. Example: <code>https://your-node.netlify.app</code>", parse_mode="HTML")
            return
        db.add_node(clean_url)
        await message.answer(f"✅ <b>API Node added:</b>\n<code>{h(clean_url)}</code>", parse_mode="HTML")
        return

    if pending == "admin_monitor_search":
        if not is_admin:
            return
        try:
            target_uid = int(text.strip())
        except ValueError:
            await message.answer("❌ Invalid User ID. Sirf numeric Telegram user ID bhejo.")
            return
        mons = db.list_monitors(target_uid)
        txt = f"🔍 <b>Auto-Monitors for User ID:</b> <code>{target_uid}</code>\n\n" + format_monitor_list(mons, False)
    ...
    if pending in {"admin_monitor_add", "user_monitor_add"}:
        ...
        username = username.split("/")[0].split("?")[0].strip().replace("@", "").lower()
        if pending == "admin_monitor_add" and not is_admin:
            return
        if not username or len(username) > 30:
            await message.answer("❌ Invalid Instagram username. Example: <code>username</code> ya <code>@username</code>", parse_mode="HTML")
            return
