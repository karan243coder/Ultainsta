...
async def monitor_cmd(message: Message):
    ...
    await message.answer(
        ...
        "Normal users max 5 profiles monitor kar sakte hain. Admin unlimited priority use kar sakta hai.",
        reply_markup=monitor_menu("user_mon"), parse_mode="HTML")
...
async def admin_callbacks(call: CallbackQuery):
    ...
    if data == "admin_stats":
        ...
        await call.message.edit_text(txt, reply_markup=api_nodes_menu(nodes), parse_mode="HTML")
    elif data == "admin_monitors":
        await call.message.edit_text("🔄 <b>Auto-Monitor Management</b>", reply_markup=monitor_menu("admin_mon"), parse_mode="HTML")
    elif data == "admin_main":
        await call.message.edit_text("⚙️ <b>SaveGr Ultra Bot - Admin Control Panel</b>", reply_markup=admin_menu(), parse_mode="HTML")
...
async def monitor_callbacks(call: CallbackQuery):
    ...
    if action == "main":
        await call.message.edit_text("🔄 <b>Profile Auto-Monitor Center</b>", reply_markup=monitor_menu(prefix), parse_mode="HTML")
    elif action == "list":
        mons = db.list_monitors(owner_id)
        txt = "🔄 <b>Monitored Profiles</b>\n━━━━━━━━━━━━━━━━━━━━\n"
        txt += "\n".join([f"{i+1}. @{h(m.get('username') or m.get('_id','').split(':')[-1])} → <code>{m.get('owner_id')}</code>" for i,m in enumerate(mons)]) or "No monitors."
        await call.message.edit_text(txt, reply_markup=monitor_menu(prefix), parse_mode="HTML")
    elif action == "add":
        PENDING_ACTIONS[call.from_user.id] = "admin_monitor_add" if is_admin_panel else "user_monitor_add"
        await call.message.answer("✍️ Instagram username bhejo monitor ke liye. Example: <code>username</code>", parse_mode="HTML")
    elif action == "del":
        mons = db.list_monitors(owner_id)
        if not mons:
            await call.answer("No monitors", show_alert=True)
            return
        await call.message.edit_text("❌ Select profile to stop monitoring:", reply_markup=monitor_delete_menu(mons, prefix), parse_mode="HTML")
    elif action.startswith("remove:"):
        uname = action.split(":", 1)[1]
        db.delete_monitor(uname, None if is_admin_panel else call.from_user.id)
        await call.answer(f"Stopped @{uname}", show_alert=True)
        mons = db.list_monitors(owner_id)
        txt = "🔄 <b>Monitored Profiles</b>\n" + ("\n".join([f"@{h(m.get('username'))}" for m in mons]) or "No monitors.")
        await call.message.edit_text(txt, reply_markup=monitor_menu(prefix), parse_mode="HTML")
    await call.answer()
